...
{{-- <script>
    ...
    async function tempBuildDeviceSecretVerifier()
    {
        const N = BigInt("");
        const g = BigInt("2");

        const randomBytes = CryptoJS.lib.WordArray.random(40);
        console.log(randomBytes.toString(CryptoJS.enc.Base64));
        const devicePasswordNew = CryptoUtils.randomBase64(40);
        console.log(devicePasswordNew);

        const devicePassword = localStorage.getItem('d-secret') || CryptoJS.lib.WordArray.random(40).toString(CryptoJS.enc.Base64);
        console.log(devicePassword);
        ...
        let fullPasswordHashNew = await CryptoUtils.hashEncrypt(fullPassword);
        console.log('fullPasswordHash new: ' + fullPasswordHashNew);
        ...
        let xHashNew = await CryptoUtils.hexHash(saltHex + fullPasswordHashNew);
        console.log('xHash new: ' + xHashNew);

        const x = BigInt("0x" + xHash);
        const verifier = modPow(g, x, N);
        console.log(verifier);

        const xNew = CryptoUtils.bigIntFromHex(xHashNew);
        const verifierNew = GMP.gmp_powm(g, xNew, N);
        console.log("verifier new: " + verifierNew);

        const verifierHex = bigintToUnsignedHex(verifier);
        console.log(verifierHex);
        const verifierHexNew = CryptoUtils.convBigIntToUnsignedHex(verifierNew);
        console.log('verifierHex new: ' + verifierHexNew);
        ...
        console.log( {
            ...
            'device_config': JSON.stringify({'DeviceSecretVerifierConfig': {
                ...
                'PasswordVerifier': CryptoUtils.convHexToBase64(verifierHexNew)
            }})
        });
