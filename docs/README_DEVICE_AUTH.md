...
Authorization: Bearer <access_token>
{
  ...
  "device_config": "<device_configuration_data>"
}

```

The `device_configuration_data` is a JSON string that must contain the object with the following structure:

```json
{
  "DeviceSecretVerifierConfig": {
    "Salt": "<salt_value_generated_for_the_device>",
    "PasswordVerifier": "<password_verifier_value_generated_for_the_device>"
  }
}
```

The `Salt` and `PasswordVerifier` values are generated using the SRP protocol and are unique to the device. These values should be securely stored on the server side and used for future device authentication attempts.

The successful response for the API call would look like this:
```json
{
  "status": "success",
  "message": "Device registered successfully.",
  "data": {
   "UserConfirmationNecessary": <boolean indicating if user confirmation is required>
  }
}
```
...
PUT /device/<device_key>
Content-Type: application/json
Accept: application/json
Authorization: Bearer <access_token>
{
  "remembered_status": <boolean true or false>
}
...
If the response is successful, the device will be deleted from the user's account. If the response is unsuccessful, an error message will be returned indicating the reason for the failure.
...
2. [*Authenticator*](#step-2-authenticator-flow-or-srp_a-generation) - Generates the SRP_A value on the client side, along with the private ephemeral value 'a'.
3. [*Verifier*](#step-3-verifier-flow) - Builds the response to the DEVICE_PASSWORD_VERIFIER challenge using the SRP_B, salt, and secret block received from the server.

#### <u>***Step 1***</u>: Login with Device
...
Use the login endpoint method to initiate the device authentication process. This method will handle the communication with AWS Cognito and return the necessary challenge parameters for the next step of the authentication flow.

It just requires an additional parameter `device_key` to be sent along with the `username` and `password` in the request body. The device key is a unique identifier for the registered device and is used to authenticate the device during the login process.
...
The server will then process this request to initiate the authentication process.

If the device key is valid and the user credentials are correct, AWS Cognito will respond with an authentication challenge `DEVICE_SRP_AUTH` that includes the necessary parameters for the next step of the authentication flow.

#### <u>***Step 2***</u>: Authenticator Flow OR SRP_A Generation
